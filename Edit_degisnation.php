  ...
  <script type="text/javascript" src="Library/js/jquery-3.6.1.min.js"></script>
      ...
      <?php include 'header.php';
       ...
       include 'sidebar.php';
        require 'DB.php'; ?>
               <?php 
       if (isset($_GET['edit'])) {
       $select=$db->prepare("SELECT * FROM degisnation WHERE id=:id");
	...
	<form action=""method="POST"  id="manage-department" style="width:700px; margin: auto;">
		...
		<div class="form-group">
			...
			<input type="text" class="form-control form-control-sm " name="degisnation" value="'.$key['Degisnation'].'"">
		</div>
		<div class="form-group">
	...
	<?php
			if (isset($_POST['Edit'])) {				 
				$Edit=$db->prepare("UPDATE degisnation SET Degisnation=:Degisnation ,Discription=:Discription WHERE id=:id");
				 $Edit->bindparam("Degisnation",$_POST['degisnation']);
				 $Edit->bindparam("Discription",$_POST['description']);
				  $Edit->bindparam("id",$_POST['Edit']);
				   if ($Edit->execute()) {
				   	echo "<script>alert('Done')</script>";
				   	   	echo "<script>window.open('http://localhost/Admin%20PageEmployee/index.php?page=designation','_self')</script>";
				   }
				   else{
				   	 echo "Erorr";
				   }
